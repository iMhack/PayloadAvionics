  ...
  if (muonFile) {
    ...
    for (int k=0;k<GLOBAL_BUF;k=k+BUF_SIZE) { //go through the buffer
      if (copybuffer[k]!=initBuff) { //stop when you reach the last written line
        ...
        if (copybuffer[k+7]>300) {
          muonString = "A"; muonString += String(7); muonString += "  "; muonString += String(timebuffer[k/BUF_SIZE]); muonString += "  "; muonString += String(copybuffer[k+7]);muonString += "  ";
          muonFile.println(muonString);}
