...
bool RH_RF69::init()
{
    if (!RHSPIDriver::init()){Serial.println("BadDriverInit");
	return false;}

    // Determine the interrupt number that corresponds to the interruptPin
    int interruptNumber = digitalPinToInterrupt(_interruptPin);
    if (interruptNumber == NOT_AN_INTERRUPT){Serial.println("BadDigitalInterruptPin");
	return false;}
    ...
    if (_deviceType == 00 ||
	_deviceType == 0xff){Serial.print("BadSPI_Read:");Serial.println(_deviceType);
	return false;}
